import { locationLabel, type AssistantResult } from './resolve';
...
export function composeAnswer(result: AssistantResult, t: Translate): string {
  ...
  // complète à « où est X ». Plusieurs : on ne choisit pas à la place de
  // l'utilisateur, on annonce le nombre et on laisse la liste trancher.
  if (entries.length === 1) {
    return t('assistant.answer.found_one', { name: entries[0].name, location: locationLabel(entries[0]) });
  }

  return t('assistant.answer.found_many', { n: entries.length, query });
}
