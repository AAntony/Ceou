...
export function ProjectForm({sourceId,onClose,onCreated,project}:{sourceId?:string;onClose:()=>void;onCreated:(id:string)=>void;project?:MovingProject}) {
 const {t}=useTranslation(); const {data:homes=[]}=useHabitations(); const command=useMovingCommand(); const offline=useIsOffline();
 ...
 return <MovingSheet title={t(project?'moving.destinationEdit':'moving.new')} onClose={()=>{if(!command.isPending)onClose();}}>
  ...
  <Button label={t(project?'common.save':'moving.create')} loading={command.isPending} disabled={offline||(!project&&!source)||(newHome&&!homeName.trim())||(!!project&&!newHome&&!destination)} onPress={submit}/>
 </MovingSheet>;
}
export function BoxForm({project,box,previous,onClose,onCreated}:{project:MovingProject;box?:MovingBox;previous?:MovingBox;onClose:()=>void;onCreated:(id:string)=>void}) {
 const {t}=useTranslation(); const command=useMovingCommand(); const offline=useIsOffline(); const id=useRef(newId());
 ...
 return <MovingSheet title={t(box?'moving.edit':'moving.newBox')} onClose={()=>{if(!command.isPending)onClose();}}>
  ...
  <View className="mt-3">{offline?<Text className="mb-3 text-label text-ink-soft">{t('moving.online')}</Text>:null}<Button label={t(box?'common.save':'moving.boxCreate')} loading={command.isPending} disabled={offline||(!!box&&!name.trim())} onPress={submit}/></View>
 </MovingSheet>;
}
