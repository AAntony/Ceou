...
export function ProjectForm({sourceId,onClose,onCreated,project}:{sourceId?:string;onClose:()=>void;onCreated:(id:string)=>void;project?:MovingProject}) {
 ...
 return <MovingSheet title={t(project?'moving.destinationEdit':'moving.new')} onClose={()=>{if(!command.isPending)onClose();}}>
  ...
  <Text className="mb-2 mt-3 text-body font-semibold text-ink">{t('moving.destination')}</Text>
  ...
  {!project?<TextField label={t('moving.date')} value={date} onChangeText={setDate} placeholder="2026-10-01"/>:null}
  <Text className="mb-3 text-label text-ink-soft">{t('moving.online')}</Text>
  <Button label={t(project?'common.save':'moving.create')} loading={command.isPending} disabled={(!project&&(!name.trim()||!source))||(newHome&&!homeName.trim())||(!!project&&!newHome&&!destination)} onPress={submit}/>
 </MovingSheet>;
}
