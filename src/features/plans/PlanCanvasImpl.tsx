...
import { ExploreLabelLayer } from './ExploreLabelLayer';
...
export const PlanCanvas = forwardRef<PlanCanvasHandle, PlanCanvasProps>(function PlanCanvas(
  ...
  // Explicit total scale matches the measured screen-space labels.
  ...
  const zoomForBounds = (
    ...
    return clampZoomState({ scale, translateX: vw / 2 - cx * scale, translateY: vh / 2 - cy * scale }, vw, vh, fitScale, readOnly);
  };
  ...
  useEffect(() => {
    if (!initializedRef.current) return;
    if (!viewportSize.width || !viewportSize.height) return;
    setZoom((z) => clampZoomState(z, viewportSize.width, viewportSize.height, minScale, readOnly));
  }, [viewportSize, minScale, readOnly]);
  ...
  const pinch = Gesture.Pinch()
    ...
    .onUpdate((event) => {
      ...
      setZoom(clampZoomState(next, viewportSize.width, viewportSize.height, minScale, readOnly));
    });
  ...
  useImperativeHandle(ref, () => ({
    ...
    recenter: () => fitToRooms(),
    focusRoom: (id) => { const geo = geoById[id]; if (geo) fitToBounds(geo.x, geo.y, geo.x + geo.width, geo.y + geo.height, 32); },
  }));
  ...
  return (
    <View
      style={{ flex: 1, overflow: 'hidden', backgroundColor: readOnly ? colors.surface : colors.sandDark }}
        ...
        <View style={{ width: viewportSize.width, height: viewportSize.height }}>
          ...
          >
            <Canvas style={{ width: WORLD_WIDTH, height: WORLD_HEIGHT }}>
              ...
              {!readOnly ? <Rect x={0} y={0} width={WORLD_WIDTH} height={WORLD_HEIGHT} color={colors.inkFaint} style="stroke" strokeWidth={2} /> : null}
              ...
              {roomVisuals.map((room) => (
                <Rect
                  ...
                  color={readOnly && room.selected ? colors.accentLight : room.color}
                  ...
                  opacity={readOnly ? (room.selected ? 1 : 0.3) : (isDark ? 1 : ROOM_FILL_OPACITY)}
                  style="fill"
                />
              ))}
              ...
              {roomVisuals.map((room) =>
                room.walls.map((wall, index) => (
                  <Line
                    ...
                    style="stroke"
                    strokeWidth={wallWidth(wall.interior) * (readOnly ? 0.65 : 1)}
                  />
                )),
              )}
          ...
          <ExploreLabelLayer rooms={roomVisuals} pins={pins} selectedId={selectedFormeId ?? highlightFormeId ?? null}
            zoom={zoom} factor={textScale} highlightedId={highlightEmplacementId}
            editingMetrics={readOnly ? undefined : PIN_METRICS[pinSize]}
            numbers={Object.fromEntries(orderedRooms(formes).map((f, index) => [f.id, index + 1]))} />
          </>
          ) : null}
        </View>
      </GestureDetector>
