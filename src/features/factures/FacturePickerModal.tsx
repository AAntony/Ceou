...
import { useFacturesARattacher } from './queries';
...
//
// LES PLUS RÉCENTES D'ABORD, et c'est le tri qui compte : un rattachement
// suit presque toujours une saisie de la minute précédente. La recherche
// n'est là que pour le cas d'après — retrouver le ticket d'un meuble acheté
// l'an dernier pour y ajouter la table assortie.
//
// CE QU'ON MONTRE DE CHAQUE FACTURE : son document, son vendeur, sa date, son
// total, et LES OBJETS QU'ELLE COUVRE DÉJÀ. Ce dernier point est celui qui
// permet de reconnaître le bon ticket quand on en a trois du même magasin le
// même jour.

type FacturePickerModalProps = {
  visible: boolean;
  objetId: string;
  onClose: () => void;
  onChoisir: (facture: { id: string; vendor: string | null }) => void;
};

export function FacturePickerModal({ visible, objetId, onClose, onChoisir }: FacturePickerModalProps) {
  ...
  // fermée. Elle vit sur la fiche d'un objet, qu'on ouvre bien plus souvent
  // qu'on ne rattache une facture.
  ...
  const renderItem = useCallback(
    ({ item }: { item: NonNullable<typeof data>[number] }) => (
      <Rangee
        ...
        onPress={() => {
          setRecherche('');
          onChoisir({ id: item.id, vendor: item.vendor });
        }}
      />
    ),
    [i18n.language, onChoisir],
  );
  ...
  return (
    <Modal visible={visible} animationType="slide" onRequestClose={fermer} statusBarTranslucent>
      <View className="flex-1 bg-sand" style={{ paddingTop: insets.top }}>
        ...
        <Text className="px-6 pb-2 text-label leading-5 text-ink-soft">{t('factures.picker.intro')}</Text>
  ...
  return (
    ...
    >
      ...
      {/* TROIS LIGNES, TOUJOURS, comme les cartes du dossier : le titre, le
          montant aligné, puis ce que la facture couvre déjà. */}
