...
- Le message « Réglementations européennes » est publié pour Céoù, d'après la capture du 17 septembre. L'affichage d'une publicité Google de test dans l'APK 1.2.1 a été confirmé sur téléphone, sans formulaire préalable. Cela valide l'affichage de l'annonce, pas encore le parcours de consentement européen ni le crédit du bonus.
- L'erreur de « Mes choix publicitaires » a été identifiée dans `client_errors` : `Privacy options form is not required.` Le bouton ignorait `privacyOptionsRequirementStatus`. Il est désormais affiché seulement si UMP renvoie `REQUIRED`, avec une nouvelle vérification à l'ouverture et après une publicité. `NOT_REQUIRED` affiche une information sans lancer le formulaire ; un état inconnu ou une panne conserve une option de nouvelle vérification. Si une première réponse reste nécessaire, le formulaire initial est présenté une seule fois. Aucun consentement n'est réinitialisé ni accordé par le correctif.
- Le motif pour lequel Google ne réclame pas de formulaire sur ce téléphone reste à vérifier (association du message à l'application, ciblage, contexte de test). Valider encore l'acceptation, le refus et la modification des choix sur un appareil pour lequel UMP demande le formulaire avant la diffusion réelle.
- Les erreurs publicitaires sont désormais distinguées (configuration du formulaire, réseau, consentement, disponibilité) et journalisées avec l'étape, sans identité publicitaire ni challenge. Si l'actualisation UMP échoue, une autorisation antérieure n'est utilisée que lorsque le SDK renvoie encore `canRequestAds=true`. Aucune publicité, même de démonstration, n'est demandée sinon.
